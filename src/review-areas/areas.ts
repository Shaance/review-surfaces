...
export const REVIEW_AREAS: ReviewArea[] = [
  ...
  area("SUB-COLLECT", "Collection and indexing", "COLLECTOR", ["src/collector/", "src/acai/", "src/core/"], "Collect Git, diff, docs, tests, and input indexes.", "deterministic collector", ["acai", "collect"]),
  area("SUB-BOOTSTRAP", "Bootstrap files", "BOOTSTRAP", ["AGENTS.md", ".agents/", "review-surfaces.config.yaml"], "Keep local source-of-truth and agent workflow files available.", "spec-first bootstrap", ["config", "bootstrap"]),
  ...
  area("SUB-EVIDENCE", "Evidence and schema validation", "EVIDENCE", ["src/evidence/", "src/schema/", "src/review-areas/"], "Represent and validate evidence references used by packet claims.", "evidence model", ["evidence"]),
  area("SUB-PRIVACY", "Privacy controls", "PRIVACY", [".review-surfacesignore", "src/privacy/"], "Exclude sensitive files and prepare redaction boundaries before remote provider use.", "privacy guard", ["privacy"]),
  area("SUB-DOGFOOD", "Dogfood loop", "DOGFOOD", ["src/dogfood/", ".review-surfaces/agent_handoff.md", "docs/dogfooding.md"], "Turn self-review findings into local product feedback and handoff.", "feedback loop", ["dogfood"]),
