...
interface BuildSectionsInput {
  ...
  // repo's own signals. One value feeds BOTH the machine clusters and the
  // tour's file categorization so they stay in agreement on any layout.
...
export function buildChangeGraphSections(input: BuildSectionsInput): { change_graph: ChangeGraph; reading_order: ReadingOrder } {
  ...
  // Clusters follow first appearance in the reading order.
...
// cross-link resolution so old-side queue anchors agree with queue_refs.
