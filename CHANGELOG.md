...
- Eval + benchmark + docs (`EVAL_HARNESS.7`, `BENCH.2`, `DISTRIBUTION.16`): the
  ...
  pinned public benchmark (`bench/manifest.json`, `node bench/run.mjs`) gains ten
  Swift/SwiftPM/iOS cases spanning all six `BENCH.2` shapes — a SwiftPM
  public-declaration change, app source + matching XCTest, a Swift Testing weakening (a
  removed `@Test` the test-weakening detector flags) plus a Swift Testing integration
  change, a package requirement/pin change, an entitlement/privacy-manifest config
  change, and a mixed source+tests+docs+generated-churn case — each annotated with
  `expected_focus` (a single intended primary file) except the deliberately-broad
  exclusion-stress case. The runner's classifier now recognizes Swift XCTest files
  (`*Tests.swift`) and the SwiftPM lock (`Package.resolved`) so the metrics are honest;
  a case whose reviewed subject IS a lock/dependency artifact opts that role out via
  `allow_top_roles`. An on-demand run holds the quality bar on them: 0 empty queues,
  0 fabricated blockers, 0 irrelevant generated/lock/binary entries in the top-5, and
  100% expected-focus recall on the annotated cases, with every pre-existing case
  keeping its pass outcome (32/32). The README and AGENTS docs publish an honest support
