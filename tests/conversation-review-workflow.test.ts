...
test("review-surfaces.CONVERSATION_REVIEW.3 a second-pass outage preserves analyzed intent and stays retryable", async () => {
  ...
  assert.match(result.analysis.summary, /stated goal: keep the retry behavior/i);
  assert.match(result.analysis.summary, /latest refinement: do not remove retries; preserve the behavior/i);
