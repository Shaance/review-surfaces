...
test("review-surfaces.REVIEWER_VALUE.4 all reviewer surfaces lead with the shared decision projection", () => {
  ...
  const surfaces = [
    ["Markdown", renderHumanReviewMarkdown(model), "## Verdict", "## Active intent", "## Decision findings", "## Review first", "## Reading order", fullDecisionSupportingText(projection.supporting_detail_counts)],
    ["HTML", renderHumanReviewHtml(model), "id=\"verdict\"", "id=\"active-intent\"", "id=\"decision-findings\"", "id=\"queue\"", "id=\"reading-order\"", fullDecisionSupportingText(projection.supporting_detail_counts)],
    ["sticky", renderStickySummary(model).markdown, "**Reviewable with attention.**", "### Active intent", "### Decision findings", "### Review first", "### Review first", compactDecisionSupportingText(projection.supporting_detail_counts)],
    ["PR comment", renderHumanPrComment(model).markdown, "**Verdict:**", "### Active intent", "### Decision findings", "### Review first", "### Review first", compactDecisionSupportingText(projection.supporting_detail_counts)]
  ] as const;
  for (const [name, rendered, verdictMarker, intentHeading, findingsHeading, queueHeading, supportingHeading, supportingText] of surfaces) {
    ...
    const queueIndex = rendered.indexOf(queueHeading);
    ...
    assert.ok(queueIndex >= findingsIndex, `${name} must render review actions after findings`);
    assert.ok(supportingIndex >= queueIndex, `${name} must move supporting machinery below review actions`);
