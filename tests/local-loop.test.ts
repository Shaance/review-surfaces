...
test("review-surfaces.LOCAL_LOOP.2 local-gate.sh runs lint, typecheck, full test, determinism-check, and the strict empty-diff self-dogfood as one command", () => {
  ...
  assert.match(localGate, /--prefer-offline/, "pack smoke prefers the checkout's cached packages while permitting missing registry metadata");
  assert.match(localGate, /--store-dir "\$PNPM_STORE_ROOT"/, "tarball install reuses the discovered store");
